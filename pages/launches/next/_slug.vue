...
<div class="border-0 rounded-2xl bg-gray-100 dark:bg-gray-700 shadow-lg">
  <div class="grid grid-cols-1 sm:grid-cols-2 sm:px-8 sm:py-12 sm:gap-x-8 md:py-16">
    ...
    <div class="col-start-1 row-start-3 space-y-3 px-4 pb-4">
      ...
      <Countdown :date="launch.results[0].net" @onClick="finish()"></Countdown>
    </div>
...
export default {
  ...
  head: {
    ...
    meta: [
      ...
      { name: 'og:title', content: 'Launches - LiftoffTv' },
      ...
      // Twitter
      { name: 'twitter:title', content: 'Launches - LiftoffTv' },
      ...
      // SEO
      { name: 'title', content: 'Launches - LiftoffTv' },
...
computed: {
    launched () {
        return this.launches.find(launched => launched.results[0].slug === this.page_slug)
    }
}
